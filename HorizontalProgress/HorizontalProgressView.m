...
#define PROGRESS_PADDING 2
#define PROGRESS_TIPLABEL_HEIGHT 30
#define PROGRESS_LeftMargin 15
...
- (void)configElementsOnAbove:(BOOL)flag {
    ...
    CGFloat unitLineWidth = (self.frame.size.width - 2 * PROGRESS_PADDING - 2 * pointRadius - 2 * PROGRESS_LeftMargin) / ([self.progressLevelArray count] - 1);
    
    for (NSInteger index = 0; index < [self.progressLevelArray count]; index++) {
        
        CGRect unitLineRect = CGRectMake(PROGRESS_PADDING + index * unitLineWidth + pointRadius + PROGRESS_LeftMargin, (self.frame.size.height - lineHeight) / 2.0, unitLineWidth, lineHeight);
        CGRect pointRect = CGRectMake(PROGRESS_PADDING + index * unitLineWidth + PROGRESS_LeftMargin, self.frame.size.height / 2.0 - pointRadius, 2 * pointRadius, 2 * pointRadius);
        ...
        if (index <= self.currentLevel) {
            [self generatePointWithFrame:pointRect achieved:YES above:flag text:[NSString stringWithFormat:@"%ld",(long)index]];
            [self generateTipLabelWithFrame:labelRect
                                   achieved:YES
                                  labelText:[self.progressLevelArray objectAtIndex:index]
                                      above:flag];
        } else {
            [self generatePointWithFrame:pointRect achieved:NO above:flag text:[NSString stringWithFormat:@"%ld",(long)index]];
            [self generateTipLabelWithFrame:labelRect
                                   achieved:NO
                                  labelText:[self.progressLevelArray objectAtIndex:index]
                                      above:flag];
